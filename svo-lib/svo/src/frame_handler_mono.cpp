...
UpdateResult FrameHandlerMono::processFirstFrame()
{
  ...
  const auto res = initializer_->addFrameBundle(new_frames_, use_gt_init_pose_);
  ...
  map_->addKeyframe(newFrame(), false);
...
UpdateResult FrameHandlerMono::processFrame()
{
  ...
  bool new_keyframe;
  if (use_RL_actions_) {
    new_keyframe = action_newkeyframe_;
  }
  else {
    new_keyframe = need_new_kf_(newFrame()->T_f_w_) 
