...
//                               (scale * x) / cell_size);
  ...
  void change_cell_size(float new_cell_size, int new_n_cols, int new_n_rows)
  {
    cell_size = new_cell_size;
    n_cols = new_n_cols;
    n_rows = new_n_rows;
    Grid new_occupancy(new_n_cols * new_n_rows, false);
    FeatureGrid new_feature_occupancy(new_n_cols * new_n_rows, Keypoint(0, 0));
    for (size_t i = 0; i < occupancy_.size(); ++i)
    {
      if (occupancy_.at(i)) {
        const Keypoint& keypoint = feature_occupancy_[i];

        const int int_x = static_cast<int>(keypoint(0));
        const int int_y = static_cast<int>(keypoint(1));
        const size_t idx = getCellIndex(int_x, int_y, 1);

        new_occupancy.at(idx) = true;
        new_feature_occupancy.at(idx) = keypoint;
      }
    }
    occupancy_ = new_occupancy;
    feature_occupancy_ = new_feature_occupancy;
  }
